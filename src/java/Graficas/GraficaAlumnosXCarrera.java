 ...
 * @author Pwned
 */
public class GraficaAlumnosXCarrera extends HttpServlet {

    Conexion conn = new Conexion();
    private static final String sql_GraficaAlumnosxCarreraC = "{call AlumnosxCarreraC()}";

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        JFreeChart chart = ChartFactory.createPieChart3D("Alumnos Por Carrera",
                getGraficaAlumnosXCarrera(), true, true, Locale.getDefault());
        ...
        graficaServlet.processRequest(request, response, "AlumnosxCarrera.jpeg", "Alumnos Por Carrera");
    ...
    private PieDataset getGraficaAlumnosXCarrera() {
    ...
    public List grafica(Connection conn) throws SQLException, ClassNotFoundException {
        ...
        try {
            cs = (CallableStatement) conn.prepareCall(sql_GraficaAlumnosxCarreraC);
            rs = cs.executeQuery();
            while (rs.next()) {
